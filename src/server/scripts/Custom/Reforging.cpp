...
//  Reforging config
// ########################################################################################
static const uint32 tokenEntry      = 49426;    // token entry
...
static const accepted_stat_format accepted_stat_types[] =
{
    ...
    {ITEM_MOD_HEALTH,       "Health",           5000},
    {ITEM_MOD_STRENGTH,     "Strength",         250},
    {ITEM_MOD_INTELLECT,    "Intellect",        250},
    {ITEM_MOD_AGILITY,      "Agility",          100},
    {ITEM_MOD_SPIRIT,       "Spirit",           250},
    {ITEM_MOD_STAMINA,      "Stamina",          500},
    {ITEM_MOD_ATTACK_POWER, "Attack power",     500},
    {ITEM_MOD_SPELL_POWER,  "Spell power",      500},
};
            ...
            do
            {
                ...
                if(sObjectMgr->_itemFakeStatStore.find(lowGUID) != sObjectMgr->_itemFakeStatStore.end() && sObjectMgr->_itemFakeStatStore[lowGUID].size() >= maxReforges)
                {
                    if(sent_error_message.find(lowGUID) != sent_error_message.end())
                        continue;
                    sLog->outError(LOG_FILTER_GENERAL,"Too many reforges for item lowGUID %u, ignoring extra reforges.", lowGUID);
